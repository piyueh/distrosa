...
def plot_gradients_1d(anspars, params, grads: dict, figdir):
    ...
    band_3 = ax.fill_between(pars, distrosa25, distrosa75, color="tab:blue", alpha=0.3, zorder=3)
    ...
    ax.set_ylabel(
        r"Derivative of Loss w.r.t. $\theta_1$ "
        r"($\frac{\partial L}{\partial \theta_1}$)"
    )
...
def plot_train_hists(params, losses, anspars, distrosahist, fdhist, figdir):
    ...
    fig, ax = pyplot.subplots(1, 1, figsize=(2.5, 2.5))
    ax.contour(
        params[..., 0], params[..., 1], losses,
        numpy.linspace(losses.min(), losses.max(), 128),
        cmap="turbo", norm="log", alpha=0.3, linewidths=0.5
    )
    ax.scatter(anspars[0], anspars[1], 20, "k", marker="^", label="Truth")
    ax.plot(distrosahist[:, 0], distrosahist[:, 1], "tab:blue", label="DistroSA")
