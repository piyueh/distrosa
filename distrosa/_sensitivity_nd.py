...
class SensitivityND:
    ...
    epsx : None | float | NDArray
        Finite difference step size(s) for spatial derivatives. If None, it is inferred
        from the gridlines. If given a 1D array, its length must be the dimensionality.
    ...
    * The floating point precision is determined by the dtype of `gridlines[0]`.
    ...
    def __init__(
        ...
        self._epsx = self._np.zeros(self._N, dtype=self._ftype)
        if epsx is None:
            self._epsx[:] = self._np.asarray([_.min()/10.0 for _ in self._dx])
        else:  # either eps is a scalar or an array
            self._epsx[:] = epsx
    ...
    def __call__(self, x: NDArray, params: NDArray) -> NDArray:
        ...
        * Parameters can still be anything that implements the array interface.
        ...
        bsize = nelms_per_1g // max(self._K) // self._N  # chunck size
    ...
    def _backend(self, x: NDArray, params: NDArray) -> NDArray:
        ...
        # treatment of epsx
        epsx: NDArray = self._epsx
        epsx2: NDArray = 2.0 * epsx
        ...
        # empty containers
        H = _np.zeros(nx+(N, N), dtype=self._ftype)
        G = _np.zeros(nx+(N, P), dtype=self._ftype)
        ...
        for i in range(N):
            ...
            for j in range(N):
                ...
                xj = x[..., j].copy()  # xk shape (Nx,)

                # positive perturb the j-th spatial dimension
                _np.add(xk[..., j], epsx[j], out=xk[..., j])
                cdfp, _ = _getcdf(self._density, xk, params, self._dx[i])

                # negative perturb the j-th spatial dimension
                _np.subtract(xk[..., j], epsx2[j], out=xk[..., j])
                cdfm, _ = _getcdf(self._density, xk, params, self._dx[i])

                # reusing cdfp mem space; cdfp = d F_i / d x_j
                _np.subtract(cdfp, cdfm, out=cdfp)
                _np.divide(cdfp, epsx2[j], out=cdfp)  # shape (Nx, Ki)

                # 1D interpolation; shape change: (Nx,), (Ki,), (Nx, Ki) -> (Nx,)
                H[..., i, j] = _minterp(x[..., i], v[i], self._dx[i], cdfp)

                # restore _x
                xk[..., j] = v[i] if i == j else xj.reshape(nx+(1,))
            ...
            for j in range(P):
                ...
                # 1D interpolation; shape change: (Nx,), (Ki,), (Nx, Ki) -> (Nx,)
                G[..., i, j] = _minterp(x[..., i], v[i], self._dx[i], cdfp)

        # solve the linear systems (avoid singular matrices)
        valid = _np.linalg.matrix_rank(H) >= N
        J[valid, :, :] = _np.linalg.solve(H[valid], G[valid])

        # apply the negative sign on the solution directly
        _np.negative(J, out=J)
