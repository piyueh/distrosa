...
class SensitivityNDInterp:
    ...
    * The floating point precision is determined by the dtype of `gridlines[0]`.
    ...
    def _construct(self, params: NDArray) -> None:
        ...
        _, cdfs = _getconditionals(density, v, dhs, params)
        ...
        # initialize arrays
        H = _np.zeros(K + (N, N), dtype=self._ftype)
        G = _np.zeros(K + (N, P), dtype=self._ftype)
        J = _np.zeros(K + (N, P), dtype=self._ftype)

        # [preparing H]: loop over each spatial direction
        for j in range(N):

            # alias for step sizes in the j-th direction
            dh = dhs[j]

            # loop over each 1D conditional direction and apply central difference
            for i in range(self._N):
                _centraldiff(cdfs[i], dh, j, out=H[..., i, j])  # internal points
                _forwarddiff(cdfs[i], dh, j, out=H[..., i, j])  # lower boundary
                _backwarddiff(cdfs[i], dh, j, out=H[..., i, j])  # upper boundary

        # [preparing G]: loop over each conditional
        for i in range(N):
            for j in range(P):  # loop over each parameter
                G[..., i, j] = (cdfsp[j][i] - cdfsm[j][i]) / eps2[j]
    ...
    def __call__(self, x: NDArray, params: NDArray) -> NDArray:
        ...
        * Parameters can still be anything that implements the array interface.
        ...
        # to hold the outputs
        Jx = _np.zeros(x.shape+(len(params),), dtype=self._ftype)
